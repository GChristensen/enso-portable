...
# Standalone bring-up test for the macOS TransparentWindow (no Enso core).
#
...
# The rectangle must be GREEN with white text; wrong colors indicate a
# pixel-format (byte order) bug.
...
from enso.platform.osx import graphics
...
class _Stepper(Foundation.NSObject):

    def initWithWindow_(self, window):
        self = objc.super(_Stepper, self).init()
        if self is None:
            return None
        self.window = window
        self.state = {"phase": "fade_out", "opacity": 255, "ticks": 0}
        return self
...
def main():
    ...
    stepper = _Stepper.alloc().initWithWindow_(window)
    Foundation.NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
        0.016, stepper, "step:", None, True)
